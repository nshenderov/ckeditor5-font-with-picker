...
import '../../theme/inputcolor.css';

import ColorInputView from './colorinputview';
import pipetteIcon from '../../theme/icons/pipette.svg';
import addColorIcon from '../../theme/icons/add.svg';
...
export default class ColorTableView extends View {
	...
	_createColorInputView() {
		const colorInputView = new ColorInputView(this.locale, 'close');
	...
	_createLabel(text){
		...
		labelView.extendTemplate({
			attributes: {
				class: ['ck', 'ck-color-grid__label', 'picker-anchor']
			}
		});
		return labelView;
	}
	...
	_appendColorInput() {
		...
		this.pickerButtonView = this._createButton( 'pick color', pipetteIcon );
		this.pickerButtonView.extendTemplate({
			attributes: {
				class: ['ck','ck-button','ck-off','ck','ck-color-grid__tile', 'ck-color-picker'],
			}
		})

		this.colorInputView = this._createColorInputView();

		this.saveButtonView = this._createButton( 'apply color', addColorIcon);
		this.saveButtonView.extendTemplate({
			attributes: {
				class: ['ck','ck-button','ck-off','ck','ck-color-grid__tile', 'ck-color-picker-save'],
			}
		})
		this.saveButtonView.on('execute', () => {
			let value = this.colorInputView.getInputValue();
			this.fire('execute', {value});
		});

		this.canvasAnchor = new LabelView(this.locale);
		this.canvasAnchor.extendTemplate({
			attributes: { class: 'picker-anchor' }
		});

		this.items.add(this.colorInputView);
		this.items.add(this.pickerButtonView);
		this.items.add(this.saveButtonView);
		this.items.add(this.canvasAnchor);

		this.colorInputView.set({
			styleElem: this.pickerButtonView.element,
			parent: this.canvasAnchor.element
		});
	}
}
